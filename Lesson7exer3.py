class Cell:
    ...
    def __init__(self, count: int):
        assert count > 0, "The number of cells must be greater 0"
        self.__count = count
    ...
    def __sub__(self, other: 'Cell'):
        self.validate_item(other)
        value = self.count - other.count
        assert value > 0, "The difference of cells is less 0"
        return Cell(value)
    ...
    def validate_item(self, other):
        assert isinstance(other, self.__class__), "Operations are allowed only between cells"
...
print(first / second)  # 3 / 2 = 1 (cells)
